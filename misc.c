...
{
    int len = nblks * BLOCK_SIZE, start = lba * BLOCK_SIZE;
    
    assert(lba >= 0 && start+len <= disk_bytes);

    lseek(disk_fd, start, SEEK_SET);
...
int block_write(char *buf, int lba, int nblks)
{
    ...
    /* write to 0 is *always* an error */
    assert(lba > 0 && start+len <= disk_bytes);
    
    lseek(disk_fd, start, SEEK_SET);
    write(disk_fd, buf, len);
    return 0;
}
